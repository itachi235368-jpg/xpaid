...
export const UsePaidFrontPage: React.FC<UsePaidFrontPageProps> = ({
  ...
  return (
    <div className="relative z-10 max-w-7xl mx-auto px-3.5 sm:px-6 lg:px-8 py-4 sm:py-12 space-y-10 sm:space-y-16 pb-28 sm:pb-12">
      
      {/* 1. Radical Dual-Pane Command Deck (Futuristic Launch Terminal + Live Holographic Card) */}
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 items-stretch pt-1 sm:pt-4">
        
        {/* Left Column: Command Launch Terminal */}
        <div className="lg:col-span-7 holo-card p-4 sm:p-8 lg:p-10 rounded-3xl space-y-5 sm:space-y-6 flex flex-col justify-between relative overflow-hidden border border-lime-500/30 shadow-2xl">
          <div className="space-y-3.5 sm:space-y-4">
            
            {/* Live Holographic Badge */}
            <div className="inline-flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-1 rounded-full bg-lime-500/10 border border-lime-500/30 text-[11px] sm:text-xs font-mono font-bold text-lime-400">
              <Radio className="w-3 h-3 sm:w-3.5 sm:h-3.5 animate-pulse text-lime-400 shrink-0" />
              <span className="truncate">FARTPAY CREATOR ROYALTY PROTOCOL</span>
            </div>

            {/* Display Headline */}
            <h1 className="text-2xl sm:text-4xl lg:text-5xl font-black text-slate-900 dark:text-white leading-[1.15] font-['Outfit']">
            ...
            <div className="space-y-2.5 sm:space-y-3 pt-1 sm:pt-2">
              <label className="text-[11px] sm:text-xs font-mono font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                <div className="relative flex-1">
                  <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-lime-400 font-bold font-mono text-sm">
                    @
                  </div>
                  <input
                    type="text"
                    ...
                    className="w-full pl-8 pr-4 py-3 sm:py-3.5 rounded-2xl bg-slate-100 dark:bg-slate-950/80 border border-slate-200 dark:border-white/10 text-slate-900 dark:text-white font-mono font-bold text-xs sm:text-sm focus:outline-none focus:border-lime-400 transition-colors shadow-inner"
                  />
                </div>
                ...
                <button
                  ...
                  className="w-full sm:w-auto px-5 sm:px-6 py-3 sm:py-3.5 rounded-2xl bg-gradient-to-r from-lime-400 via-emerald-400 to-cyan-400 hover:from-lime-300 hover:to-cyan-300 text-slate-950 font-black text-xs sm:text-sm flex items-center justify-center gap-2 transition-all cursor-pointer shadow-lg shadow-lime-500/20 active:scale-95 shrink-0 uppercase tracking-wider font-mono"
                >
              ...
              {/* Mobile Smooth Horizontally Scrollable Quick Pick Chips */}
              <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar py-1 text-xs">
                <span className="text-slate-400 font-mono text-[10px] sm:text-[11px] shrink-0">Popular:</span>
                {FEATURED_CREATORS.map(c => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setInputHandle(c.handle);
                    }}
                    className={`px-2.5 py-1 rounded-xl text-xs font-semibold whitespace-nowrap transition-all cursor-pointer shrink-0 ${
                      inputHandle.toLowerCase() === c.handle.toLowerCase()
                        ? 'bg-lime-400 text-slate-950 font-black shadow-xs scale-105'
          ...
          <div className="grid grid-cols-3 gap-1.5 sm:gap-2 pt-4 sm:pt-6 border-t border-slate-200 dark:border-white/10">
            <div className="p-2 sm:p-3 rounded-xl bg-slate-100 dark:bg-black/30 border border-slate-200 dark:border-white/5 text-center">
              <div className="text-[9px] sm:text-[10px] text-slate-400 font-mono truncate">CREATOR SHARE</div>
              <div className="text-sm sm:text-lg font-black text-lime-400 font-mono">95% USD</div>
            </div>
            <div className="p-2 sm:p-3 rounded-xl bg-slate-100 dark:bg-black/30 border border-slate-200 dark:border-white/5 text-center">
              <div className="text-[9px] sm:text-[10px] text-slate-400 font-mono truncate">BUY & BURN</div>
              <div className="text-sm sm:text-lg font-black text-amber-400 font-mono">5% SOL</div>
            </div>
            <div className="p-2 sm:p-3 rounded-xl bg-slate-100 dark:bg-black/30 border border-slate-200 dark:border-white/5 text-center">
              <div className="text-[9px] sm:text-[10px] text-slate-400 font-mono truncate">SETTLEMENT</div>
              <div className="text-sm sm:text-lg font-black text-cyan-400 font-mono">&lt; 0.05s</div>
            </div>
          </div>
        </div>
      ...
      <section className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-200 dark:border-white/10 pb-4">
          <div className="flex items-center gap-1.5 sm:gap-2 overflow-x-auto no-scrollbar py-1 -mx-1 px-1">
            <button
              onClick={() => { playFartSound('random'); setActiveTabSection('tokens'); }}
              className={`px-3.5 sm:px-4 py-2 rounded-xl font-bold text-xs sm:text-sm whitespace-nowrap transition-all cursor-pointer ${
                activeTabSection === 'tokens'
                  ? 'bg-gradient-to-r from-lime-400 via-emerald-400 to-cyan-400 text-slate-950 shadow-md font-black scale-105'
            ...
            <button
              onClick={() => { playFartSound('random'); setActiveTabSection('simulator'); }}
              className={`px-3.5 sm:px-4 py-2 rounded-xl font-bold text-xs sm:text-sm whitespace-nowrap transition-all cursor-pointer ${
            ...
            <button
              onClick={() => { playFartSound('random'); setActiveTabSection('architecture'); }}
              className={`px-3.5 sm:px-4 py-2 rounded-xl font-bold text-xs sm:text-sm whitespace-nowrap transition-all cursor-pointer ${
          ...
          <button
            type="button"
            onClick={() => { playFartSound('rip'); onLaunchClick(); }}
            className="hidden sm:inline-flex px-4 py-2 rounded-xl bg-slate-900 dark:bg-slate-800 hover:bg-slate-800 dark:hover:bg-slate-700 text-white font-bold text-xs transition-all border border-slate-700 items-center justify-center gap-1.5 cursor-pointer self-start sm:self-auto shadow-sm"
          >
